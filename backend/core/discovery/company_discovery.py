...
class ExaCompanyDiscovery(CompanyDiscoveryService):
    ...
    async def discover_companies(
        ...
        """Discover companies using comprehensive sources including Exa, YC, and Techstars."""
        logger.info(f"🚀 Starting discovery: {limit} companies from Exa + YC + Techstars...")
        
        all_companies = []
        
        # Determine which sources to use
        use_exa = sources is None or "exa" in sources
        use_yc = sources is None or "yc" in sources or "ycombinator" in sources
        use_techstars = sources is None or "techstars" in sources
        
        # Collect companies from all sources in parallel
        tasks = []
        
        if use_exa:
            # Use 70% of limit for Exa since it's the primary source
            exa_limit = int(limit * 0.7)
            tasks.append(self.find_companies(
                limit=exa_limit,
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    logger.info(f"✅ {source_name}: {len(result)} companies")
        
        except Exception as e:
            logger.error(f"Error in parallel company discovery: {e}")
            # Fallback to sequential discovery
            if use_exa:
                try:
                    exa_companies = await self.find_companies(
                        limit=int(limit * 0.7),
                        categories=categories,
                        regions=regions
                    )
                    all_companies.extend(exa_companies)
                    logger.info(f"✅ Exa (fallback): {len(exa_companies)} companies")
                except Exception as exa_error:
                    logger.error(f"Exa discovery failed: {exa_error}")
            
            if use_yc:
                try:
                    yc_companies = await self.discover_yc_ai_companies()
                    all_companies.extend(yc_companies)
                    logger.info(f"✅ YC (fallback): {len(yc_companies)} companies")
        ...
        # Deduplicate companies across all sources
        logger.info(f"🔄 Deduplicating {len(all_companies)} companies from all sources...")
        unique_companies = self._deduplicate_companies(all_companies)
        
        # Apply limit to final results
        final_companies = unique_companies[:limit]
        
        logger.info(f"✅ Discovery complete: {len(final_companies)} unique companies from {len(tasks)} sources")
...
"""
    ...
    async def discover_yc_ai_companies(self) -> List[Company]:
        """Discover AI/ML companies from Y Combinator using unofficial API."""
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status != 200:
                    logger.error(f"Failed to fetch YC companies: {response.status}")
                    return companies
                
                yc_companies = await response.json()
                logger.info(f"📊 Fetched {len(yc_companies)} total YC companies")
                ...
                for yc_company in yc_companies:
                    try:
                        # Check if company is AI/ML related
                        if self._is_ai_ml_company(yc_company, ai_ml_keywords):
    ...
    def _is_ai_ml_company(self, yc_company: dict, ai_ml_keywords: List[str]) -> bool:
        """Check if a YC company is AI/ML related based on multiple fields."""
        # Fields to check for AI/ML keywords
        fields_to_check = [
            yc_company.get('one_liner', ''),
            yc_company.get('long_description', ''),
            yc_company.get('industry', ''),
            yc_company.get('subindustry', ''),
            ' '.join(yc_company.get('industries', [])),
            ' '.join(yc_company.get('tags', []))
        ]
    ...
    async def _convert_yc_to_company(self, yc_company: dict) -> Optional[Company]:
        """Convert YC company data to Company object."""
        try:
            ...
            company = Company(
                uuid=f"yc_{yc_company.get('slug', hash(yc_company.get('name', '')))}",
                name=clean_text(yc_company.get('name', '')),
    ...
    async def _convert_techstars_to_company(self, techstars_company: dict) -> Optional[Company]:
        """Convert Techstars company data to Company object."""
        try:
            # Process website URL
            ...
            # Create Company object
            company = Company(
                uuid=f"techstars_{techstars_company.get('slug', hash(techstars_company.get('name', '')))}",
                name=clean_text(techstars_company.get('name', '')),
                ...
                investors=["Techstars"],
                categories=[techstars_company.get('industry', 'AI/ML')],
                city=clean_text(city),
                region=clean_text(region),
                country=clean_text(country),
                ai_focus="AI/ML",  # All companies from this endpoint are AI/ML
