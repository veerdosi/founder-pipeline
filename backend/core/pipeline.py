...
class InitiationPipeline:
    ...
    async def run(
        ...
        try:
            ...
            # Clear running stage on pipeline failure
            checkpoint_manager.clear_running_stage(self.job_id)
            raise
    ...
    async def _enrich_companies_checkpointed(self, csv_file_path: str, limit: Optional[int], force_restart: bool):
        ...
        try:
            # Set current running stage for progress tracking
            checkpoint_manager.set_running_stage(self.job_id, stage_name)
            ...
            # Clear running stage after completion
            checkpoint_manager.clear_running_stage(self.job_id)
            ...
            # Clear running stage on failure
            checkpoint_manager.clear_running_stage(self.job_id)
            raise
            ...
            if enriched.profiles:
                ...
                try:
                    ...
                    if checkpoint_success:
                        logger.info(f"💾 Incremental checkpoint saved")
                    ...
                    if failure_count >= max_consecutive_failures:
                        ...
                        # Clear running stage before stopping due to failures
                        checkpoint_manager.clear_running_stage(self.job_id)
